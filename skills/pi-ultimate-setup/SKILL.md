...
## 6.5 使用技巧提示（非配置项，结束报告时顺带提醒）

体检和增强完成后，向用户简要提示以下「用熟即丝滑」的能力（不写入配置，只是告知）：

- **会话命名**：`/name <名字>` 给会话起名（或启动时 `pi --name "..."`），避免会话列表全是第一条消息、难以找回。
- **会话找回**：`/resume` 或 `pi -r` 打开选择器，直接输入文字可搜索全部消息正文（不止标题，支持 `re:<正则>` 与 `"短语"` 精确搜索）；`Ctrl+S` 循环排序（Threaded 树状 → Recent 最近优先 → Relevance 相关度）；`Ctrl+N` 只看已命名会话；`Ctrl+R` 重命名；`Ctrl+D` 删除。
- **分支与克隆**：`/tree` 回跳历史节点、`/fork` 从旧消息新开会话、`/clone` 改前备份、`/compact` 压缩超长会话。

详细见仓库 `docs/tips-session-management.md`。
