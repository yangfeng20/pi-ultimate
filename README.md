...
- AI 在 Windows 上查中文日志 → 乱码，你反复解释原因，下个会话它又踩一遍
- AI 一搜索就扫 node_modules，卡 3 分钟，结果全是噪音
- AI 跑了条会卡死的命令，20 分钟后你回来，会话还挂在那儿
- 轮到你值班，人肉盯 bug 系统、盯工单群，漏了背锅
- 创建的定时巡检任务被隔壁窗口的会话"抢走"执行，输出驴唇不对马嘴

**这些全部可以一次性解决。** 完整的场景描述与解法对照 → [docs/pain-points.md](docs/pain-points.md)

| 阶段 | 模块 | 能力 | 解决的痛点 |
|---|---|---|---|
| **基础层**（丝滑化） | M1 | 命令执行环境 | Windows 中文乱码、引号嵌套地狱 |
...
每个模块对应 `docs/` 一篇文档，痛点与使用场景详见 [docs/pain-points.md](docs/pain-points.md)。**建议直接用"一句话安装"让 AI 引导你完成配置，文档作为深入理解与视频教程素材。**
